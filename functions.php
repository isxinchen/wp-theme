...
function wp_pagenavi($range = 9){
	...
	echo '<div class="page-navigation"><span class="total-page">共有' . $max_page . '页</span>';
	if($paged != 1){
		echo '<a href="' . get_pagenum_link(1) . '" class="first-page" title="跳转到首页">首页</a>';
	}
	...
	if($max_page > $range){
		if($paged < $range){
			for($i = 1; $i <= ($range + 1); $i++){
				...
				if($i==$paged)
				echo ' class="current-page"';echo '>' . $i . '</a>';
			}
		}
	...
	next_posts_link('下一页');
	if($paged != $max_page){
		echo '<a href="' . get_pagenum_link($max_page) . '" class="last-page" title="跳转到最后一页">末页</a>';
	}
	echo '</div>';
	}
}
